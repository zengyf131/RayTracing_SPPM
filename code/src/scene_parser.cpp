...
void SceneParser::parsePerspectiveCamera() {
    ...
    int height = readInt();
    getToken(token);
    assert (!strcmp(token, "maxNearbyPhoton"));
    maxNearbyPhoton = readInt();
    getToken(token);
    assert (!strcmp(token, "maxNearbyDist_square"));
    maxNearbyDist_square = readFloat();
    getToken(token);
    assert (!strcmp(token, "sppm_iteration"));
    sppm_iteration = readInt();
    getToken(token);
    assert (!strcmp(token, "}"));
...
void SceneParser::parseLights() {
    ...
    while (num_lights > count) {
        ...
        if (!strcmp(token, "MaterialIndex")) {
            ...
            continue;
        } else if (strcmp(token, "DirectionalLight") == 0) {
...
Light *SceneParser::parseDirectionalLight() {
    ...
    float power = readFloat();
    getToken(token);
    assert (!strcmp(token, "photonAmount"));
    int photonAmount = readInt();
    getToken(token);
    assert (!strcmp(token, "}"));
    return new DirectionalLight(direction, color, position, radius, power, photonAmount, current_material);
}
...
Light *SceneParser::parsePointLight() {
    ...
    getToken(token);
    assert (!strcmp(token, "}"));
    return new PointLight(position, color, radius, power, photonAmount, current_material);
}
...
Light *SceneParser::parseSpotLight() {
    ...
    float radius = readFloat();
    getToken(token);
    assert (!strcmp(token, "maxAngle"));
    float maxAngle = readFloat();
    getToken(token);
    assert (!strcmp(token, "power"));
    float power = readFloat();
    getToken(token);
    ...
    getToken(token);
    assert (!strcmp(token, "}"));
    return new SpotLight(direction, color, position, radius, maxAngle, power, photonAmount, current_material);
}
...
void SceneParser::parseMaterials() {
    ...
    while (num_materials > count) {
        ...
        } else if (!strcmp(token, "LightMaterial")) {
            materials[count] = parseLightMaterial();
        } else {
...
Material *SceneParser::parseMaterial() {
    ...
    while (true) {
        ...
        if (strcmp(token, "diffuseColor") == 0) {
            ...
            getToken(filename);
            tex = Image::LoadTGA(filename);
        } else if (strcmp(token, "normal_texture") == 0) {
            // Optional: read in texture and draw it.
            getToken(filename);
            n_tex = Image::LoadTGA(filename);
        } else if (strcmp(token, "diffuseRate") == 0) {
            diffuseRate = readFloat();
    ...
    auto *answer = new Material(diffuseColor, specularColor, shininess, diffuseRate, specularRate, refractRate, refractIndex, tex, n_tex);
    return answer;
}

Material *SceneParser::parseLightMaterial() {
    char token[MAX_PARSER_TOKEN_LENGTH];
    char filename[MAX_PARSER_TOKEN_LENGTH];
    filename[0] = 0;
    Vector3f diffuseColor(1, 1, 1), specularColor(0, 0, 0);
    ...
    auto *answer = new Material(diffuseColor, specularColor, shininess, diffuseRate, specularRate, refractRate, refractIndex, nullptr, nullptr, isLight, lightColor);
    return answer;
}
...
Square *SceneParser::parseSquare() {
    ...
    assert (!strcmp(token, "{"));
    getToken(token);
    assert (!strcmp(token, "minPoint"));
    Vector3f minPoint = readVector3f();
    getToken(token);
    assert (!strcmp(token, "maxPoint"));
    Vector3f maxPoint = readVector3f();
    getToken(token);
    assert (!strcmp(token, "}"));
    assert (current_material != nullptr);
    return new Square(minPoint, maxPoint, current_material);
}
